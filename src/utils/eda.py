...
def run_eda() -> None:
    """
    Runs Exploratory Data Analysis (EDA) on the 10% scarce train split.
    Saves a sample image plot and a summary markdown report.
    """
    ...
    # 2. Stats
    mean_val: float = images.mean() / 255.0
    std_val: float = images.std() / 255.0
    logger.info(f"Dataset stats: Mean={mean_val:.4f}, Std={std_val:.4f}")
    ...
    with open('reports/eda_summary.md', 'w') as f:
        ...
        f.write(f"- Image Stats: Mean={mean_val:.4f}, Std={std_val:.4f}\n")
